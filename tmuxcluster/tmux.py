#!/usr/bin/env python
import os
from .utils import ossystem, wait_for_file
import tmuxcluster.utils as u
import shlex
import functools
...
def exec_on_node(cmds, host='localhost'):
    ...
    cmds = [_decide_node(cmd) for cmd in cmds] if isinstance(cmds, list) else _decide_node(cmds)
    ossystem(cmds)
...
class Job(object):
    ...
    def make_tasks(self):
        for task_id, (window, script) in enumerate(zip(self.windows, self.task_scripts)):
            self.tasks.append(Task(window, self, task_id, install_script=script, task_node=self.job_node))
...
class Task(object):
    ...
    * no pane is used.
    ...
    def __init__(self, window, job, task_id, install_script, task_node):
        ...
        # Path
        self.taskdir = f"{TASKDIR_PREFIX}/{job._run.name}.{job.name}.{self.id}.{0}"
        self.scratch = self.taskdir + '/scratch'
        self.cmds = []

        self.cmds += [f"mkdir -p {self.taskdir}", f"rm -Rf   {self.scratch}", f"mkdir -p {self.scratch}"]

        # Run counter of the task
        self._run_counter = 0
        self.run(f'cd {self.taskdir}')
    ...
    def run(self, cmd):
        ...
        # Log cmd
        # u.write(cmd_in_fn, cmd+'\n')
        # exec_on_node(, task_node)

        # modified_cmd = '%s ; echo $? > %s' % (cmd, cmd_out_fn)
        modified_cmd = cmd
        self.cmds.append(f'tmux send-keys -t {self.window} {shlex.quote(modified_cmd)} Enter')
